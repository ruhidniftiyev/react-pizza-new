...
const Home = () => {
  ...
  const aq = useSelector((state) => state.basket.totalPrice);
  console.log(aq);
  console.log();

  const [pizzas, setPizzas] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [oizzasCount, setPizzasCount] = useState(0);
    ...
    <>
      ...
      <div className="content__items">
        {isLoading
          ...
          : pizzas.map((item) => <PizzaBlock {...item} key={item.id} count={item.id}/>)}
      </div>
    </>
  );
};
